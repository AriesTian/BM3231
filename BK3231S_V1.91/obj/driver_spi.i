...
{
    do{ if(!(len<=127)) { bprintf("ASSERT!SPI,%d",281); while(1); } }while(0);
    do{ if(!(tx_data!=0)) { bprintf("ASSERT!SPI,%d",282); while(1); } }while(0);
    do{ if(!(rx_data!=0)) { bprintf("ASSERT!SPI,%d",283); while(1); } }while(0);
    ...
    if((*((volatile unsigned long *) (0x00F00200 + 0x00) ))&(0x01 << 22))  
    {  
        spi_trans_start();
    }
    spi_int_enable();
...
void BK_SPI_Test_Master_Tx(void)
{
    ...
    SPI_transmit_data(spi_tx_data,spi_rx_data, 127);
...
{
#line 350 "src\\Driver\\Driver_spi.c"
}

#line 382 "src\\Driver\\Driver_spi.c"
