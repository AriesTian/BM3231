...
void FAST_CALL Host_Loop()
{
	...
	if(config->work_mode < 2)
    {
        if(uart_send_cnt == 0x00)
        {
            ...
            else if((uart_rx_buff_tailor == uart_rx_buff_temp) && (uart_cnt_index > 0))/*&&(sys_flag&FLAG_CONNECTED)*/
            {
                ...
                uart_rx_buff_cnt++;
                if( uart_rx_buff_cnt >= 200)//200//30)超时置位uart_send_cnt
                {
    ...
    if(sys_flag&FLAG_CONNECTED)
    {
        if((uart_send_cnt || spi_send_cnt) && (sys_flag & FLAG_SPP_CONNECTED))//spp 发送数据
        {
        ...
        else if(uart_send_cnt || spi_send_cnt)//(tx_buff_count)//串口数据接收完成/spi接收完数据
        {
